...
import type { RockMesh, RockColliderMesh } from './RockMesh.js';

export class RockMeshDecimator {
  /**
   * Simplify a mesh using vertex clustering.
   * 
   * Algorithm:
  ...
  decimate(mesh: RockMesh, gridResolution: number = 16): RockColliderMesh {
    ...
    const cellMap = new Map<string, number[]>(); // cellKey -> vertexIndices
    ...
    const oldToNewIndex = new Map<number, number>(); // old vertex index -> new vertex index
    ...
    for (let t = 0; t < triangleCount; t++) {
      ...
      // Skip degenerate triangles (all 3 vertices collapsed to same point)
