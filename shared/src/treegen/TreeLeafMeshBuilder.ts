/**
 * Convert greedy-meshed tree quads to indexed triangle meshes for leaf trigger colliders.
 * Only processes leaf quads - wood is ignored.
 */
...
export class TreeLeafMeshBuilder {
  /**
   * Convert TreeQuad[] to an indexed triangle mesh (leaves only).
  ...
  buildFromQuads(quads: TreeQuad[]): TreeMesh {
    // Filter to leaf quads only (material === 2)
    const leafQuads = quads.filter(q => q.material === 2);
