/**
 * Greedy meshing for cloud voxels.
 * Merges coplanar solid faces into larger quads to minimize polygon count.
 * Similar to TreeGreedyMesher but operates on density (> 0 = solid).
 */

import type { CloudVoxelGrid } from './CloudVoxelGrid.js';
import { CLOUD_VOXEL_SIZE } from './CloudVoxelGrid.js';

export interface CloudQuad {
  x: number;
  y: number;
  z: number;
  width: number;
...
export class CloudGreedyMesher {
  private grid: CloudVoxelGrid;

  constructor(grid: CloudVoxelGrid) {
    this.grid = grid;
  }

  generateMesh(): CloudQuad[] {
    const quads: CloudQuad[] = [];
  ...
  private meshAxis(axis: 'x' | 'y' | 'z'): CloudQuad[] {
    const quads: CloudQuad[] = [];
  ...
  private createQuad(
    ...
    return {
      x: coords.x * CLOUD_VOXEL_SIZE,
      y: coords.y * CLOUD_VOXEL_SIZE,
      z: coords.z * CLOUD_VOXEL_SIZE,
      width,
      height,
      axis,
      positive,
