/**
 * Greedy meshing for bush voxels.
 * Merges coplanar solid faces into larger quads to minimize polygon count.
 * Same approach as CloudGreedyMesher — operates on density (> 0 = solid).
 */

import type { BushVoxelGrid } from './BushVoxelGrid.js';
import { BUSH_VOXEL_SIZE } from './BushVoxelGrid.js';

export interface BushQuad {
  x: number;
  y: number;
  z: number;
  width: number;
...
export class BushGreedyMesher {
  private grid: BushVoxelGrid;

  constructor(grid: BushVoxelGrid) {
    this.grid = grid;
  }

  generateMesh(): BushQuad[] {
    const quads: BushQuad[] = [];
  ...
  private meshAxis(axis: 'x' | 'y' | 'z'): BushQuad[] {
    const quads: BushQuad[] = [];
  ...
  private createQuad(
    ...
    return {
      x: coords.x * BUSH_VOXEL_SIZE,
      y: coords.y * BUSH_VOXEL_SIZE,
      z: coords.z * BUSH_VOXEL_SIZE,
      width,
      height,
      axis,
      positive,
