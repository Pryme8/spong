...
import { Scene, Mesh, MeshBuilder, DynamicTexture, Color3, MirrorTexture, Plane } from '@babylonjs/core';
...
export class LevelWaterManager {
  ...
  private createMirrorReflection(): void {
    ...
    // Use all scene meshes for render list
    this.mirrorTexture.renderList = [];
    for (const mesh of this.scene.meshes) {
      // Skip the water plane itself
      if (mesh !== this.waterPlane && mesh.isEnabled() && mesh.isVisible) {
  ...
  /**
   * Update mirror reflection render list (call after adding new meshes to scene).
   */
  refreshMirrorRenderList(): void {
    ...
    console.log(`[WaterManager] Mirror render list updated: ${this.mirrorTexture.renderList.length} meshes`);
  }
