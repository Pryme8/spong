...
export class AudioManager {
   ...
   */
  async loadSounds(soundManifest: { name: string; path: string; channel?: string; maxInstances?: number; spatial?: boolean; startTime?: number }[]): Promise<void> {
   ...
   */
  play(soundName: string, options: PlaySoundOptions = {}): boolean {
    if (!this.initialized) {
      return false;
    }
    ...
    const instance = soundDef.instances.find((inst) => !inst.inUse);
    if (!instance) {
      return false;
    }
    ...
    // Connect: source -> panner (if spatial) -> gain -> channel -> master
  ...
  /**
   * Update listener position for spatial audio (call each frame).
   */
  updateListener(x: number, y: number, z: number, forwardX?: number, forwardY?: number, forwardZ?: number): void {
    this.listenerX = x;
    this.listenerY = y;
    this.listenerZ = z;
