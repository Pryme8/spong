...
export class InputManager {
  ...
  private shouldSkipRightClick: (() => boolean) | null = null;
  ...
  private setupPointerObservable() {
    ...
    // Prevent context menu on right-click - store handler for cleanup
    const canvas = this.scene.getEngine().getRenderingCanvas();
    if (canvas) {
      this.contextMenuHandler = (e: MouseEvent) => {
        if (this.isMobile) return; // Allow context menu on mobile
        e.preventDefault();
      };
  ...
  dispose() {
    ...
    this.shouldSkipRightClick = null;
  }
}
